...
const BoardComponent = ({ game, handlePieceClick }: Props, ) => {
    ...
    return (
        <div className="board bg-amber-100 relative">
            ...
            <div>
                {/* Outer */}
                <Mill id="0-6" mill={game.getBoard().getMill("0-6")} position="w-[21.15rem] h-1 top-7.5 left-7.5" />
                <Mill id="0-42" mill={game.getBoard().getMill("0-42")} position="w-1 h-[21.15rem] top-7.5 left-7.5" />
                <Mill id="6-48" mill={game.getBoard().getMill("6-48")} position="w-1 h-[21.15rem] top-7.5 right-7.5" />
                <Mill id="42-48" mill={game.getBoard().getMill("42-48")} position="w-[21.15rem] h-1 bottom-7.5 left-7.5" />
                {/* Middle */}
                ...
                {/* Intersecting */}
                <Mill id="3-17" mill={game.getBoard().getMill("3-17")} position="w-1 h-[7.2rem] top-7.5 left-[12.4rem]" />
                <Mill id="21-23" mill={game.getBoard().getMill("21-23")} position="w-[7.2rem] h-1 top-[12.4rem] left-7.5" />
                <Mill id="25-27" mill={game.getBoard().getMill("25-27")} position="w-[7.2rem] h-1 top-[12.4rem] right-7.5" />
                <Mill id="31-45" mill={game.getBoard().getMill("31-45")} position="w-1 h-[7.2rem] bottom-7.5 left-[12.4rem]" />
            </div>
            {/* Render Pieces */}
            <div className="grid grid-cols-7 gap-6 w-100 p-4 shadow-[0_0px_0px_16px_rgba(217,119,6,1)] relative rounded-sm">
